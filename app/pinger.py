...
Down criteria: every 60s a burst of ping_count pings is sent with a 1s timeout
each. If ALL pings in the burst fail, the host is declared DOWN immediately.
If any succeed, the host is UP and latency is the avg RTT of the burst.
...
def ping_host(host: str, count: int = 5, timeout: int = 1):
    ...
    try:
        ...
        if result.returncode == 0:
            # At least one ping succeeded — extract avg RTT
...
class PingScheduler:
    def __init__(self, max_workers: int = 50, ping_count: int = 5, ping_timeout: int = 1):
        """
        ping_count:   pings per burst (all must fail to declare down)
        ping_timeout: per-ping timeout in seconds
        A host is declared DOWN if all pings in a burst fail, UP immediately
        if any ping in a burst succeeds.
        """
    ...
    def start(self):
        ...
        self._thread.start()
        print(f"[pinger] Started — {self.ping_count} pings/burst, {self.ping_timeout}s timeout each")
            ...
            for leaf in leaves:
                ...
                with self._lock:
                    if path in self._in_flight:
                        continue
                    if now - self._last_ping.get(path, 0) < interval:
                        continue
        ...
        try:
            ...
            with self._lock:
                if is_up:
                    ...
                    # All pings in burst failed — immediately down
                    ...
                    if first_time_down:
                        print(f"[pinger] {host} ({path}): all {self.ping_count} pings failed — marking DOWN")
                        threading.Thread(
